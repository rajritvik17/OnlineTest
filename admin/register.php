...
if(!empty($_POST))
{
	if($_POST['name']!='' && $_POST['email']!='' && $_POST['username']!='' && $_POST['pwd']!='')
	{
		$res=$conn->query('SELECT * FROM `super_admin` where username="'.$_POST['username'].'"');
		if($res->num_rows)
		{
			$msg = '<h2 class="text-danger">Username already exist!</h2>';
		}
		else{
		if($_POST['pwd']==$_POST['pwd1'])
		{
			$query="INSERT INTO `super_admin` (`name`, `email`, `username`, `password`) VALUES ('".$_POST['name']."', '".$_POST['email']."', '".$_POST['username']."', '".md5($_POST['pwd'])."');";
			$conn->query($query);
			if($conn->insert_id){
				$msg = '<h2 class="text-danger">Registration successful!</h2>';
				unset($_POST);
			}else{
				$msg = '<h2 class="text-danger">Registration failed!</h2>';
			}
			$conn->close();
		}
		else
		{
			$msg = '<h2>Password doesnot match!</h2>';
		}
		}
	}
}
                ...
                <form name="loginform" id="loginform" action="" method="post">
                    <p>
                        <label for="user_login">Full Name<br />
                            <input type="text" name="name" id="user_login" class="input" value="<?php echo !empty($_POST['name'])?$_POST['name']:''; ?>" size="20" /></label>
                    </p>
                    <p>
                        <label for="user_login">Email<br />
                            <input type="text" name="email" id="user_login" class="input" value="<?php echo !empty($_POST['email'])?$_POST['email']:''; ?>" size="20" /></label>
                    </p>
                    <p>
