...
if(!empty($_POST))
{
	...
	{
		$result=$conn->query('SELECT * FROM `super_admin` where username="'.$_POST['username'].'" AND password="'.md5($_POST['pwd']).'"');
		if(!empty($result->num_rows))
		{
			...
			$_SESSION['image'] = '';
			header("location: ".$base_url."admin/dashboard.php?success=Login success");
		}
		else
		{
			header("location: ".$base_url."admin/index.php?error=Wrong credential!");
		}
		$conn->close();
	}
	else{
		$msg='<h2 class="text-danger">Fill All Credentials...</h2>';
	}
}
?>
